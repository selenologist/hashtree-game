...
fn generate_index<P: AsRef<Path>>(path: P, uri: &str) -> io::Result<String>{
    use std::ffi::OsStr;
...
fn http_get<P: AsRef<Path>>(handle: &Handle, path: P, uri: &str) -> io::Result<Response>{
    use std::io::Read;
    use std::ffi::OsStr; 
...
impl FileThread{
    ...
    fn handle<P: AsRef<Path>>(handle: &Handle, base_path: P, request: Request) -> Response{
        ...
        let result = match method{
            Method::Get => http_get(handle, path, uri),
